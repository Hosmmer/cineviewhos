...
import { createFuncion, updateFuncion, fetchAdminFuncion, fetchAdminSalas } from "@/services/reservationService";
...
function AdminFuncionForm() {
  ...
  const mutation = useMutation({
    mutationFn: () => {
      const data = { movie: movieId, sala: salaId, start_time: new Date(startTime).toISOString() };
  ...
  return (
    <div className="max-w-lg">
      <h1 className="text-2xl font-bold text-white mb-6">{isEdit ? "Editar Funcion" : "Nueva Funcion"}</h1>

      {error && (
        <div className="mb-4 p-3 bg-red-900/50 border border-red-700 text-red-400 text-sm rounded-lg">{error}</div>
      )}

      <div className="space-y-4">
        <div>
          <label className="block text-sm text-gray-400 mb-1">Pelicula</label>
          <select value={movieId} onChange={(e) => setMovieId(Number(e.target.value))} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-red-500">
            <option value={0}>Seleccionar...</option>
            {movies.map((m: MovieList) => <option key={m.id} value={m.id}>{m.title}</option>)}
          </select>
        </div>

        <div>
          <label className="block text-sm text-gray-400 mb-1">Sala</label>
          <select value={salaId} onChange={(e) => setSalaId(Number(e.target.value))} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-red-500">
            <option value={0}>Seleccionar...</option>
            {salas.map((s: SalaDetail) => <option key={s.id} value={s.id}>{s.name} ({s.rows}&times;{s.cols})</option>)}
          </select>
        </div>

        <div>
          <label className="block text-sm text-gray-400 mb-1">Fecha y Hora</label>
          <input type="datetime-local" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-red-500" />
        </div>
