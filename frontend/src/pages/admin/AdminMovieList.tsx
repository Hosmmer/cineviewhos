  ...
  if (error) {
    return (
      <div className="text-center py-12">
        ...
        <button
          onClick={() => queryClient.invalidateQueries({ queryKey: ['admin-movies'] })}
        ...
        <table className="w-full min-w-[900px]">
          <thead>
            <tr className="border-b border-gray-800">
              <th className="text-left text-xs font-medium text-gray-400 uppercase tracking-wider px-4 py-3 w-16">Poster</th>
              <th className="text-left text-xs font-medium text-gray-400 uppercase tracking-wider px-4 py-3">Title</th>
              <th className="text-left text-xs font-medium text-gray-400 uppercase tracking-wider px-4 py-3">Genre</th>
              <th className="text-left text-xs font-medium text-gray-400 uppercase tracking-wider px-4 py-3">Director</th>
              <th className="text-left text-xs font-medium text-gray-400 uppercase tracking-wider px-4 py-3">Year</th>
              ...
              <th className="text-left text-xs font-medium text-gray-400 uppercase tracking-wider px-4 py-3">Active</th>
          ...
          <tbody className="divide-y divide-gray-800">
            {data && data.results.length > 0 ? (
              data.results.map((movie) => (
                <tr key={movie.id} className="hover:bg-gray-800/50">
                  <td className="px-4 py-3">
                    {movie.poster ? (
                      <img src={movie.poster} alt={movie.title} className="w-12 h-[72px] object-cover rounded" />
                    ) : (
                  ...
                  <td className="px-4 py-3 text-sm text-gray-400">{movie.genre_name}</td>
                  <td className="px-4 py-3 text-sm text-gray-400">{movie.director}</td>
                  <td className="px-4 py-3 text-sm text-gray-400">{movie.release_year}</td>
                  <td className="px-4 py-3 text-sm text-gray-400">{movie.duration_minutes} min</td>
                  <td className="px-4 py-3 text-sm text-gray-400">{formatPrice(movie.price)}</td>
                  <td className="px-4 py-3">
              ...
              <tr>
                <td colSpan={9} className="px-4 py-12 text-center text-gray-500">No movies found. Create your first movie to get started.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {data && data.count > 20 && (
        <div className="flex items-center justify-between mt-4">
          <p className="text-sm text-gray-500">
            Showing {((Number(page) - 1) * 20) + 1}-{Math.min(Number(page) * 20, data.count)} of {data.count}
          </p>
          <div className="flex gap-2">
            <button
              disabled={!data.previous}
              onClick={() => setSearchParams((prev) => { prev.set('page', String(Number(page) - 1)); return prev })}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 max-w-sm w-full mx-4">
            ...
            <p className="text-sm text-gray-400 mb-6">
              This will deactivate the movie. It will no longer appear on the home page but can be reactivated later.
            </p>
