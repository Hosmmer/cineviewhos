...
const validationSchema = yup.object({
  new_password: yup.string().min(8, 'Mínimo 8 caracteres').required('Contraseña es requerida'),
...
function PasswordResetConfirmPage() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setFieldError }) => {
      try {
        await resetPasswordConfirm(uid!, token!, values.new_password, values.re_new_password)
        setSuccess(true)
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-900 px-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-sm">
        ...
        <form onSubmit={formik.handleSubmit} className="bg-gray-800/80 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6 sm:p-8 space-y-5 shadow-2xl">
          ...
          <div>
            <label htmlFor="new_password" className="block text-sm font-medium text-gray-300 mb-1.5">Nueva Contraseña</label>
            <input
              id="new_password" name="new_password" type="password"
              onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.new_password}
            ...
            {formik.touched.new_password && formik.errors.new_password && (
              <p className="text-red-400 text-xs mt-1">{formik.errors.new_password}</p>
            )}
          </div>

          <div>
            <label htmlFor="re_new_password" className="block text-sm font-medium text-gray-300 mb-1.5">Confirmar Contraseña</label>
            <input
              id="re_new_password" name="re_new_password" type="password"
              onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.re_new_password}
              className="w-full px-4 py-2.5 bg-gray-700/50 text-white text-sm rounded-lg border border-gray-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent placeholder-gray-400 transition-all"
              placeholder="••••••••"
            />
            {formik.touched.re_new_password && formik.errors.re_new_password && (
              <p className="text-red-400 text-xs mt-1">{formik.errors.re_new_password}</p>
            )}
          </div>
