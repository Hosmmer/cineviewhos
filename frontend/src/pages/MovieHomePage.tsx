    ...
    <div className="space-y-8">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-gray-800 to-gray-900 border border-gray-700/50 p-8 sm:p-12">
        ...
        <div className="relative z-10">
          ...
          <p className="text-gray-400 text-lg mb-6">Tu plataforma de peliculas</p>
          {health && (
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm font-medium ${health.status === 'ok' ? 'text-green-400' : 'text-red-400'}`}>
                {health.status === 'ok' ? 'Backend conectado' : 'Backend offline'}
              </span>
            </div>
          )}
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-600 text-sm mt-1">Check back soon for new releases.</p>
        </div>
      )}
    </div>
  )
