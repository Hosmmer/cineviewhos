...
import { useModules } from "@/features/system/hooks/useModules";
import {
  ChevronDown,
  PanelLeftClose,
  PanelLeftOpen,
...
const SidebarGroup = memo(function SidebarGroup({
  ...
  if (!hasChildren && module.route) {
    return (
      ...
      >
        <ModuleIcon name={module.icon} className="w-5 h-5 shrink-0" />
        ...
        <div className={`${collapsed ? "mx-2 pb-1" : "ml-9"} mt-0.5 mb-1 space-y-0.5`}>
          {module.children.map((child) => {
            ...
            return (
              ...
              >
                <ModuleIcon name={child.icon} className="w-4 h-4 shrink-0" />
