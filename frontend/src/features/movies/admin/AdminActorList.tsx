...
import { fetchAdminActors, deleteActor } from "@/features/movies/api/actors.api";
import type { Actor } from "@/features/movies/types/actor.types";
import ConfirmModal from "@/components/ui/ConfirmModal";

function AdminActorList() {
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        <p className="text-red-400 mb-4">Failed to load actors.</p>
        <button
          onClick={() =>
            queryClient.invalidateQueries({ queryKey: ["admin-actors"] })
          }
  ...
  return (
    <div>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-800">
            {actors && actors.length > 0 ? (
              actors.map((actor) => (
                <tr key={actor.id} className="hover:bg-gray-800/50">
                  <td className="px-4 py-3 text-sm text-white">{actor.name}</td>
      ...
      <ConfirmModal
        open={deleteId !== null}
        title="Delete Actor"
        message="Are you sure you want to delete this actor? The actor will be deactivated and hidden from the public catalog."
        confirmLabel="Delete"
        cancelLabel="Cancel"
        isPending={deleteMutation.isPending}
        error={
          deleteMutation.error
            ? (deleteMutation.error as { response?: { data?: { detail?: string } } })?.response?.data?.detail || "Failed to delete actor."
            : null
        }
