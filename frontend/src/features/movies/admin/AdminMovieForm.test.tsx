...
vi.mock("@/features/movies/api/genres.api", () => ({
  fetchAdminGenres: vi.fn().mockResolvedValue([{ id: 1, name: "Action" }]),
}));
vi.mock("@/features/movies/api/directors.api", () => ({
  fetchAdminDirectors: vi.fn().mockResolvedValue([]),
}));
vi.mock("@/features/movies/api/authors.api", () => ({
  fetchAdminAuthors: vi.fn().mockResolvedValue([]),
}));
vi.mock("@/features/movies/api/actors.api", () => ({
  fetchAdminActors: vi.fn().mockResolvedValue([]),
}));

import { createMovie, updateMovie } from "@/features/movies/api/movies.api";
