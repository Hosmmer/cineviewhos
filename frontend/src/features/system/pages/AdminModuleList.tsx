...
import { createModule, updateModule, deleteModule, fetchRoles, fetchModulesFlat } from "@/features/system/api/modules.api";
import type { Module, Role } from "@/features/system/types/system.types";
  ...
  const renderRow = (mod: Module, isChild: boolean) => {
    ...
    return (
      <tr key={mod.id} className={`${isChild ? "bg-gray-900/50" : "hover:bg-gray-800/50"}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            {hasChildren ? (
              <button onClick={() => toggleExpand(mod.id)} className="flex items-center gap-2 text-sm text-white hover:text-red-400 transition-colors">
                ...
                {!isChild && <ModuleIcon name={mod.icon} className="w-4 h-4 text-gray-400 shrink-0" />}
        ...
        <td className="hidden sm:table-cell px-4 py-3">{!hasChildren && <ModuleIcon name={mod.icon} className="w-4 h-4 text-gray-400" />}</td>
      ...
      <ConfirmModal
        ...
        error={
          deleteMutation.error
            ? (deleteMutation.error as { response?: { data?: { detail?: string } } })?.response?.data?.detail || "Error al eliminar."
            : null
        }
