...
export async function loginUser(credentials: LoginCredentials): Promise<{ user: User; tokens: AuthTokens }> {
  const tokenRes = await djangoApi.post<AuthTokens>('/auth/jwt/create/', credentials)
...
export async function resetPasswordConfirm(uid: string, token: string, new_password: string, re_new_password: string): Promise<void> {
...
export function loadAuthData(): { user: User | null; tokens: AuthTokens | null } {
  try {
