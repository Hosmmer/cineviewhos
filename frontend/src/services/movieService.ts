...
import type { Movie, MovieList, Genre, MovieFormData, GenreFormData, PaginatedResponse } from '@/types/movies'
...
export async function fetchMovies(params?: Record<string, string | number>): Promise<PaginatedResponse<MovieList>> {
  const res = await djangoApi.get<PaginatedResponse<MovieList>>('/movies/', { params })
  return res.data
}
...
export async function fetchAdminMovies(params?: Record<string, string | number>): Promise<PaginatedResponse<MovieList>> {
  const res = await djangoApi.get<PaginatedResponse<MovieList>>('/admin/movies/', { params })
  return res.data
}
...
export async function updateMovie(id: number, data: MovieFormData): Promise<Movie> {
  const formData = toFormData(data)
...
export async function updateGenre(id: number, data: GenreFormData): Promise<Genre> {
